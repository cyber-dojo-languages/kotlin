...
# different kotlin. Reading it from the image instead would compare the image
# against itself and pass whatever the move brought in.
readonly EXPECTED=2.4

# Asks the installed compiler, so the gate fails when the kotlin the image can
