...
# The image resolves the newest stable kotlin when it is built and records it
...
readonly VERSION_REGEX='"kotlin":"([0-9.]+)"'
if [[ ! ${VERSIONS} =~ ${VERSION_REGEX} ]]; then
  echo "VERSION ERROR: /versions.json has no kotlin property"
...
# build time is not the kotlin the image can actually run.
